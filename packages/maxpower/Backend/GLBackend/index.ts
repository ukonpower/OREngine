...
import { Backend, BackendBuffer, BackendCubeFrameBuffer, BackendCubeTexture, BackendFrameBuffer, BackendFrameBufferOption, BackendProgram, BackendTexture, BackendVAO, TimerQueryResult } from '..';
import { Blending, DrawType } from '../../Material';

// canvasとWebGL2コンテキストを用意してGLBackendを組み立てる
export const createBackend = (): GLBackend => {

	const canvas = document.createElement( "canvas" );

	return new GLBackend( canvas.getContext( "webgl2", { antialias: false, preserveDrawingBuffer: true } )! );

};

// WebGLのリソース生成・描画コマンド発行・ステート管理を担うバックエンド。
// Rendererはパス編成に専念し、生のgl呼び出しはここへ集約する。
export class GLBackend implements Backend {
	...
	public createBuffer(): BackendBuffer {
		...
		return new GLP.GLPowerBuffer( this.gl );
	...
	public bindRenderTarget( renderTarget: BackendFrameBuffer | null, viewPort?: GLP.Vector | null, canvasSize?: GLP.Vector ) {

		const target = renderTarget as GLP.GLPowerFrameBuffer | null;
	...
	public blit( read: BackendFrameBuffer | null, draw: BackendFrameBuffer | null, width: number, height: number, linear?: boolean, restrictColor0?: boolean ) {

		const gl = this.gl;
		const readTarget = read as GLP.GLPowerFrameBuffer | null;
		const drawTarget = draw as GLP.GLPowerFrameBuffer | null;
	...
	public draw( program: BackendProgram, vao: BackendVAO, drawType: DrawType, blending: Blending, queryName?: string ) {

		const glVao = vao as GLP.GLPowerVAO;

		( program as GLP.GLPowerProgram ).use( ( program ) => {
