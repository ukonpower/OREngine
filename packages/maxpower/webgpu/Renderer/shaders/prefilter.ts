...
/*-------------------------------
	envMap の事前フィルタ（PMREM）

	webgl側の pmrem.fs（Hammersley + GGX importance sampling）を移植したもの。
	ただし出力先は3x2の面アトラスではなく、キューブテクスチャのミップレベル。
	アトラスと pmrem.module.glsl の getPmremFace / getPmremUV / getPmremMip は
	「WebGL2でキューブのミップを明示指定して引く」ための回避策なので、
	WebGPUのキューブテクスチャ＋textureSampleLevelで素直に置き換わる。

	roughness とサンプル数はミップごとに override 定数で差し込むため、
	パイプラインはミップ数ぶん、bind group は面数ぶんで足りる。
-------------------------------*/
