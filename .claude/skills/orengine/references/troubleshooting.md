...
| メッセージ | 原因と対処 |
|---|---|
| `エディタのタブが無いので headless Chromium で開こうとしましたが、失敗しました` | Playwright の Chromium が入っていない（`npx playwright install chromium`）。GPU の無い環境（Linux CI 等）では headless は使えないので、ユーザーにエディタページを開いてもらう |
| `headless Chromium で開いたエディタが …ms 以内にタブとして登録されませんでした` | headless のページが読み込めていない（エディタのビルドエラー等）。`npm run dev` のログを確認するか、ユーザーにエディタページを開いてもらってエラーを見る |
| `保存に失敗しました` / `保存が …ms 以内に終わりませんでした` | headless での書き込み後のファイル保存が失敗した。dev サーバーのログを確認する。変更がファイルに反映されていない可能性があるので `git diff scenes/` で確かめる |
...
**CLI で入れた変更が消えた**: ユーザーのタブ（`status` の `connection: "tab"`）では CLI の書き込みは保存されない。保存（Ctrl+S）の前に full-reload（`scenes/<name>.json` / `editor.json` の書き換え、コンポーネントファイルの追加・編集）が走ると消える。`status` の `unsaved` で未保存の変更の有無を確認できる。やり直してからユーザーに Ctrl+S を依頼する。
