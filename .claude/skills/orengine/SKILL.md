...
- `--time T` は「時刻 T-1/60 → T の2ステップだけ進めて描いた状態」で、「T まで再生した状態」ではない。時刻で決まるもの（BLidge アニメーション・uTime を使うシェーダー）と、直前ステップとの差で決まる速度（モーションブラー）は正確に出るが、GPUCompute・シミュレーションのように経過を積み上げるものは正しく出ない（shot 1回で2ステップ進む）。`--time` を省くとタブの今の時刻で1ステップだけ進める
